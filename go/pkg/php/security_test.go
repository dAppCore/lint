...
func TestRunFilesystemSecurityChecks_EnvInPublic(t *T) {
	...
	// Create public/.env
	publicDir := PathJoin(dir, "public")
	RequireResultOK(t, Mkdir(publicDir, 0755))
	RequireResultOK(t, WriteFile(PathJoin(publicDir, ".env"), []byte("SECRET=leaked"), 0644))
...
func TestRunFilesystemSecurityChecks_GitInPublic(t *T) {
	...
	gitDir := PathJoin(dir, "public", ".git")
	RequireResultOK(t, MkdirAll(gitDir, 0755))
...
func TestRunSecurityChecks_URLAddsHeaderCheck(t *T) {
	...
	AssertTrue(t, Contains(headerCheck.Message, "Missing headers"))
...
func setupSecurityFixture(t *T, dir string, envContent string) {
	...
	RequireResultOK(t, WriteFile(PathJoin(dir, ".env"), []byte(envContent), 0o644))

	composerBin := PathJoin(dir, "composer")
	RequireResultOK(t, WriteFile(composerBin, []byte("#!/bin/sh\ncat <<'JSON'\n{\"advisories\":{}}\nJSON\n"), 0o755))
...
func TestSecurity_RunSecurityChecks_Good(t *T) {
	subject := RunSecurityChecks
	if subject == nil {
		t.FailNow()
	}
	marker := "RunSecurityChecks:Good"
	if marker == "" {
		t.FailNow()
	}
}

func TestSecurity_RunSecurityChecks_Bad(t *T) {
	subject := RunSecurityChecks
	if subject == nil {
		t.FailNow()
	}
	marker := "RunSecurityChecks:Bad"
	if marker == "" {
		t.FailNow()
	}
}

func TestSecurity_RunSecurityChecks_Ugly(t *T) {
	subject := RunSecurityChecks
	if subject == nil {
		t.FailNow()
	}
	marker := "RunSecurityChecks:Ugly"
	if marker == "" {
		t.FailNow()
	}
}
