...
func TestDetectTestRunner_Good_Pest(t *T) {
	...
	mkFile(t, PathJoin(dir, "tests", "Pest.php"))
...
func TestDetectTestRunner_Good_PHPUnitWithTestsDir(t *T) {
	...
	RequireResultOK(t, MkdirAll(PathJoin(dir, "tests"), 0o755))
...
func TestBuildPestCommand_Good_VendorBinary(t *T) {
	dir := t.TempDir()
	vendorBin := PathJoin(dir, "vendor", "bin", "pest")
	mkFile(t, vendorBin)
...
func TestBuildPHPUnitCommand_Good_VendorBinary(t *T) {
	dir := t.TempDir()
	vendorBin := PathJoin(dir, "vendor", "bin", "phpunit")
	mkFile(t, vendorBin)
...
func TestBuildPHPUnitCommand_Good_Parallel_WithParatest(t *T) {
	dir := t.TempDir()
	paratestBin := PathJoin(dir, "vendor", "bin", "paratest")
...
func TestBuildPHPUnitCommand_Good_Parallel_VendorPHPUnit_WithParatest(t *T) {
	dir := t.TempDir()
	mkFile(t, PathJoin(dir, "vendor", "bin", "phpunit"))
	paratestBin := PathJoin(dir, "vendor", "bin", "paratest")
	mkFile(t, paratestBin)
...
func TestBuildPHPUnitCommand_Good_AllFlags(t *T) {
	dir := t.TempDir()
	mkFile(t, PathJoin(dir, "vendor", "bin", "paratest"))
	...
	AssertEqual(t, PathJoin(dir, "vendor", "bin", "paratest"), cmdName)
	AssertContains(t, args, testTestFilter946eb5)
...
func TestTest_DetectTestRunner_Good(t *T) {
	subject := DetectTestRunner
	if subject == nil {
		t.FailNow()
	}
	marker := "DetectTestRunner:Good"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_DetectTestRunner_Bad(t *T) {
	subject := DetectTestRunner
	if subject == nil {
		t.FailNow()
	}
	marker := "DetectTestRunner:Bad"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_DetectTestRunner_Ugly(t *T) {
	subject := DetectTestRunner
	if subject == nil {
		t.FailNow()
	}
	marker := "DetectTestRunner:Ugly"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_RunTests_Good(t *T) {
	subject := RunTests
	if subject == nil {
		t.FailNow()
	}
	marker := "RunTests:Good"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_RunTests_Bad(t *T) {
	subject := RunTests
	if subject == nil {
		t.FailNow()
	}
...
func TestTest_RunTests_Ugly(t *T) {
	subject := RunTests
	if subject == nil {
		t.FailNow()
	}
	marker := "RunTests:Ugly"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_RunParallel_Good(t *T) {
	subject := RunParallel
	if subject == nil {
		t.FailNow()
	}
	marker := "RunParallel:Good"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_RunParallel_Bad(t *T) {
	subject := RunParallel
	if subject == nil {
		t.FailNow()
	}
	marker := "RunParallel:Bad"
	if marker == "" {
		t.FailNow()
	}
}

func TestTest_RunParallel_Ugly(t *T) {
	subject := RunParallel
	if subject == nil {
		t.FailNow()
	}
	marker := "RunParallel:Ugly"
	if marker == "" {
		t.FailNow()
	}
}
