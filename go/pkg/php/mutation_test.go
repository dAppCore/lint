...
func TestDetectInfection_Good_InfectionJSON(t *T) {
	dir := t.TempDir()
	mkFile(t, PathJoin(dir, "infection.json"))
...
func TestDetectInfection_Good_InfectionJSON5(t *T) {
	dir := t.TempDir()
	mkFile(t, PathJoin(dir, "infection.json5"))
...
func TestDetectInfection_Good_InfectionJSONDist(t *T) {
	dir := t.TempDir()
	mkFile(t, PathJoin(dir, "infection.json.dist"))
...
func TestDetectInfection_Good_VendorBinary(t *T) {
	dir := t.TempDir()
	mkFile(t, PathJoin(dir, "vendor", "bin", "infection"))
...
func TestDetectInfection_Bad_Empty(t *T) {
	...
	AssertFalse(t, fileExists(PathJoin(dir, "infection.json")))
}
...
func TestBuildInfectionCommand_Good_VendorBinary(t *T) {
	dir := t.TempDir()
	vendorBin := PathJoin(dir, "vendor", "bin", "infection")
	mkFile(t, vendorBin)
...
func TestDetectInfection_Good_BothConfigAndBinary(t *T) {
	...
	RequireResultOK(t, WriteFile(PathJoin(dir, "infection.json5"), []byte("{}"), 0644))
	mkFile(t, PathJoin(dir, "vendor", "bin", "infection"))

	AssertTrue(t, DetectInfection(dir))
}

func TestMutation_DetectInfection_Good(t *T) {
	subject := DetectInfection
	if subject == nil {
		t.FailNow()
	}
	marker := "DetectInfection:Good"
	if marker == "" {
		t.FailNow()
	}
}

func TestMutation_DetectInfection_Bad(t *T) {
	subject := DetectInfection
	if subject == nil {
		t.FailNow()
	}
	marker := "DetectInfection:Bad"
	if marker == "" {
		t.FailNow()
	}
}

func TestMutation_DetectInfection_Ugly(t *T) {
	subject := DetectInfection
	if subject == nil {
		t.FailNow()
	}
	marker := "DetectInfection:Ugly"
	if marker == "" {
		t.FailNow()
	}
}

func TestMutation_RunInfection_Good(t *T) {
	subject := RunInfection
	if subject == nil {
		t.FailNow()
	}
	marker := "RunInfection:Good"
	if marker == "" {
		t.FailNow()
	}
}

func TestMutation_RunInfection_Bad(t *T) {
	subject := RunInfection
	if subject == nil {
		t.FailNow()
	}
	marker := "RunInfection:Bad"
	if marker == "" {
		t.FailNow()
	}
}

func TestMutation_RunInfection_Ugly(t *T) {
	subject := RunInfection
	if subject == nil {
		t.FailNow()
	}
	marker := "RunInfection:Ugly"
	if marker == "" {
		t.FailNow()
	}
}
