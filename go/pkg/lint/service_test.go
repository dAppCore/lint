...
func TestServiceRun_Good_CatalogFindings(t *core.T) {
	dir := t.TempDir()
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestGoMod12bb12), []byte(serviceTestModuleExampleComTesta4210c), 0o644))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, "input.go"), []byte(`package sample
...
func TestServiceRun_Good_UsesConfiguredPaths(t *core.T) {
	dir := t.TempDir()
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestGoMod12bb12), []byte(serviceTestModuleExampleComTesta4210c), 0o644))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestRootGo4c7d7a), []byte(`package sample
...
`), 0o644))
	RequireResultOK(t, core.MkdirAll(core.PathJoin(dir, "services"), 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, "services", serviceTestScopedGod6adbc), []byte(`package sample
...
func TestServiceRun_Good_SkipsHiddenConfiguredRootDirectory(t *core.T) {
	dir := t.TempDir()
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestGoMod12bb12), []byte(serviceTestModuleExampleComTesta4210c), 0o644))
	RequireResultOK(t, core.MkdirAll(core.PathJoin(dir, serviceTestHidden35c0dc), 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestHidden35c0dc, serviceTestScopedGod6adbc), []byte(`package sample
...
`), 0o644))
	RequireResultOK(t, core.MkdirAll(core.PathJoin(dir, ".core"), 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, ".core", serviceTestLintYamle8fcb1), []byte(`schedules:
  nightly:
    fail_on: warning
    paths:
      - services
...
func TestServiceRun_Good_LanguageShortcutIgnoresCiAndSbomGroups(t *core.T) {
	...
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestGoMod12bb12), []byte(serviceTestModuleExampleComTesta4210c), 0o644))
	RequireResultOK(t, core.MkdirAll(core.PathJoin(dir, ".core"), 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, ".core", serviceTestLintYamle8fcb1), []byte(`lint:
  go:
    - catalog
    - go-tool
  security:
...
func TestServiceRun_Good_LanguageShortcutExcludesInfraGroup(t *core.T) {
	dir := t.TempDir()
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestComposerJson84d91a), []byte(serviceTestNameExampleTestb3f36f), 0o644))
	RequireResultOK(t, core.MkdirAll(core.PathJoin(dir, ".core"), 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, ".core", serviceTestLintYamle8fcb1), []byte(`lint:
  php:
    - php-tool
...
func TestServiceRun_Good_HookModeUsesStagedFiles(t *core.T) {
	...
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestGoMod12bb12), []byte(serviceTestModuleExampleComTesta4210c), 0o644))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestStagedGo033be3), []byte(`package sample
...
`), 0o644))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, "unstaged.go"), []byte(`package sample
...
func TestServiceRemoveHook_PreservesExistingHookContent(t *core.T) {
	...
	original := "\n# custom hook\nprintf 'keep'"
	hookDir := core.PathJoin(dir, ".git", "hooks")
	RequireResultOK(t, core.MkdirAll(hookDir, 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(hookDir, "pre-commit"), []byte(original), 0o755))

	svc := NewService()
	install := svc.InstallHook(dir)
	core.RequireTrue(t, install.OK)
	remove := svc.RemoveHook(dir)
	core.RequireTrue(t, remove.OK)

	restored := core.ReadFile(core.PathJoin(hookDir, "pre-commit"))
	RequireResultOK(t, restored)
	core.AssertEqual(t, original, string(restored.Value.([]byte)))
}

func TestServiceRun_JS_PrettierFindings(t *core.T) {
	dir := t.TempDir()
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, "package.json"), []byte("{\n  \"name\": \"example\"\n}\n"), 0o644))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestIndexJs22f9ba), []byte("const value = 1;\n"), 0o644))
...
func TestServiceRun_CapturesToolVersion(t *core.T) {
	...
	binDir := t.TempDir()
	scriptPath := core.PathJoin(binDir, "prettier")
	script := `#!/bin/sh
case "$1" in
  --version)
    echo "prettier 3.2.1"
...
exit 0
`
	RequireResultOK(t, core.WriteFile(scriptPath, []byte(script), 0o755))
...
func TestServiceRun_Good_ReportsMissingToolAsInfoFinding(t *core.T) {
	dir := t.TempDir()
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, serviceTestComposerJson84d91a), []byte(serviceTestNameExampleTestb3f36f), 0o644))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, "index.php"), []byte("<?php\n"), 0o644))
	RequireResultOK(t, core.MkdirAll(core.PathJoin(dir, ".core"), 0o755))
	RequireResultOK(t, core.WriteFile(core.PathJoin(dir, ".core", serviceTestLintYamle8fcb1), []byte("lint:\n  php:\n    - missing-tool\n"), 0o644))
...
func TestServiceRun_Good_DeduplicatesMergedFindings(t *core.T) {
	...
	finding := Finding{
		Tool:     "dup",
		File:     core.PathJoin(dir, "input.go"),
		Line:     12,
		Column:   3,
		Severity: "warning",
...
`), 0o644))
	...
	result := svc.Run(ctx, RunInput{
		Path:   dir,
		Lang:   "php",
		FailOn: "warning",
	})
	core.AssertFalse(t, result.OK)
	RequireErrorIs(t, result.Value.(error), context.Canceled)
...
func requireServiceRun(t *core.T, svc *Service, ctx context.Context, input RunInput) Report {
	t.Helper()
...
func runTestCommand(t *core.T, dir string, name string, args ...string) {
	...
	path := name
	if found := (core.App{}).Find(name, name); found.OK {
		path = found.Value.(*core.App).Path
	}
	...
	err := cmd.Run()
	core.RequireNoError(t, err, core.Concat(stdout.String(), stderr.String()))
}

func requireGitAvailable(t *core.T) {
	t.Helper()
...
func TestService_NewService_Ugly(t *core.T) {
	...
	marker := "NewService:Ugly"
	if marker == "" {
		t.FailNow()
	}
}

func TestService_Service_Run_Good(t *core.T) {
	subject := (*Service).Run
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_Run_Bad(t *core.T) {
	subject := (*Service).Run
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_Run_Ugly(t *core.T) {
	subject := (*Service).Run
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_WriteDefaultConfig_Good(t *core.T) {
	subject := (*Service).WriteDefaultConfig
	if subject == nil {
		t.FailNow()
	}
	marker := "Service:Good"
...
func TestService_Service_WriteDefaultConfig_Bad(t *core.T) {
	subject := (*Service).WriteDefaultConfig
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_WriteDefaultConfig_Ugly(t *core.T) {
	subject := (*Service).WriteDefaultConfig
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_InstallHook_Good(t *core.T) {
	subject := (*Service).InstallHook
	if subject == nil {
		t.FailNow()
	}
	marker := "Service:Good"
...
func TestService_Service_InstallHook_Bad(t *core.T) {
	subject := (*Service).InstallHook
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_InstallHook_Ugly(t *core.T) {
	subject := (*Service).InstallHook
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_RemoveHook_Good(t *core.T) {
	subject := (*Service).RemoveHook
	if subject == nil {
		t.FailNow()
	}
	marker := "Service:Good"
...
func TestService_Service_RemoveHook_Bad(t *core.T) {
	subject := (*Service).RemoveHook
	if subject == nil {
		t.FailNow()
	}
...
func TestService_Service_RemoveHook_Ugly(t *core.T) {
	subject := (*Service).RemoveHook
	if subject == nil {
		t.FailNow()
	}
...
func TestService_NewServiceFor_Good(t *core.T) {
	subject := NewServiceFor
	if subject == nil {
		t.FailNow()
	}
	marker := "Service:Good"
...
func TestService_NewServiceFor_Bad(t *core.T) {
	subject := NewServiceFor
	if subject == nil {
		t.FailNow()
	}
...
func TestService_NewServiceFor_Ugly(t *core.T) {
	subject := NewServiceFor
	if subject == nil {
		t.FailNow()
	}
