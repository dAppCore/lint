...
func (s *CoverageStore) Append(snap CoverageSnapshot) core.Result {
	var snapshots []CoverageSnapshot
	load := s.Load()
	if load.OK {
		snapshots = load.Value.([]CoverageSnapshot)
	} else {
		err, _ := load.Value.(error)
		if !isNotExistError(err) {
			return core.Fail(core.E(coverageCoveragestoreAppend1a31c1, "load snapshots", err))
		}
	}
	...
	if !r.OK {
		return core.Fail(core.E(coverageCoveragestoreAppend1a31c1, "marshal snapshots", r.Value.(error)))
	}
	data := r.Value.([]byte)

	if err := coreio.Local.Write(s.Path, string(data)); err != nil {
		return core.Fail(core.E(coverageCoveragestoreAppend1a31c1, "write "+s.Path, err))
	}
	return core.Ok(nil)
}

// Load reads all snapshots from the store.
func (s *CoverageStore) Load() core.Result {
	...
	if r := core.JSONUnmarshal([]byte(raw), &snapshots); !r.OK {
		return core.Fail(core.E("CoverageStore.Load", "parse "+s.Path, r.Value.(error)))
	}
	return core.Ok(snapshots)
}

// Latest returns the most recent snapshot, or nil if the store is empty.
func (s *CoverageStore) Latest() core.Result {
	loaded := s.Load()
	if !loaded.OK {
		err, _ := loaded.Value.(error)
		if isNotExistError(err) {
			return core.Ok((*CoverageSnapshot)(nil))
		}
		return loaded
	}
	snapshots := loaded.Value.([]CoverageSnapshot)
	if len(snapshots) == 0 {
		return core.Ok((*CoverageSnapshot)(nil))
	}
...
func ParseCoverProfile(data string) core.Result {
...
func ParseCoverOutput(output string) core.Result {
	snap := CoverageSnapshot{
		Timestamp: time.Now(),
		Packages:  make(map[string]float64),
	}
