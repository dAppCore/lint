...
func TestConfig_ResolveConfigPath_Good(t *core.T) {
	...
	core.AssertEqual(t, core.CleanPath(core.PathJoin(projectPath, ".core", configTestLintYaml34d630), "/"), ResolveConfigPath(projectPath, ""))
	core.AssertEqual(
		t,
		core.CleanPath(core.PathJoin(projectPath, "config", configTestLintYaml34d630), "/"),
		ResolveConfigPath(projectPath, core.PathJoin("config", configTestLintYaml34d630)),
	)
}

func TestConfig_ResolveConfigPath_Bad(t *core.T) {
	path := ResolveConfigPath("", "")
	core.AssertEqual(t, core.CleanPath(core.PathJoin(".core", configTestLintYaml34d630), "/"), path)
	core.AssertContains(t, path, DefaultConfigPath)
}

func TestConfig_ResolveConfigPath_Ugly(t *core.T) {
	absolute := core.PathJoin(t.TempDir(), "nested", configTestLintYaml34d630)
...
func TestConfig_LoadProjectConfig_Good(t *core.T) {
	...
	write := core.WriteFile(core.PathJoin(dir, ".core", configTestLintYaml34d630), []byte(`output: sarif
fail_on: warning
paths:
  - src
exclude:
...
`), 0o644)
	...
	core.RequireTrue(t, result.OK)
	loaded := result.Value.(projectConfigResult)
	cfg := loaded.Config
	path := loaded.Path
	core.AssertEqual(t, core.PathJoin(dir, ".core", configTestLintYaml34d630), path)
...
func TestConfig_LoadProjectConfig_Bad(t *core.T) {
	...
	loaded := result.Value.(projectConfigResult)
	cfg := loaded.Config
	core.AssertEmpty(t, loaded.Path)
	core.AssertEqual(t, DefaultConfig().Output, cfg.Output)
	core.AssertEqual(t, DefaultConfig().FailOn, cfg.FailOn)
}
...
func TestConfig_LoadProjectConfig_Ugly(t *core.T) {
	...
	mkdir := core.MkdirAll(core.PathJoin(dir, ".core"), 0o755)
	core.RequireTrue(t, mkdir.OK)
	write := core.WriteFile(core.PathJoin(dir, ".core", configTestLintYaml34d630), []byte("{not: yaml"), 0o644)
	core.RequireTrue(t, write.OK)
...
func TestConfig_ResolveSchedule_Good(t *core.T) {
	...
	result := ResolveSchedule(cfg, "nightly")
	core.RequireTrue(t, result.OK)
...
func TestConfig_ResolveSchedule_Bad(t *core.T) {
	result := ResolveSchedule(DefaultConfig(), "missing")
	core.AssertFalse(t, result.OK)
...
func TestConfig_ResolveSchedule_Ugly(t *core.T) {
	result := ResolveSchedule(DefaultConfig(), "")
	core.RequireTrue(t, result.OK)
	schedule, _ := result.Value.(*Schedule)
	core.AssertNil(t, schedule)
	core.AssertEqual(t, (*Schedule)(nil), schedule)
}

func TestConfig_DefaultConfigYAML_Bad(t *core.T) {
	subject := DefaultConfigYAML
	if subject == nil {
		t.FailNow()
	}
	marker := "DefaultConfigYAML:Bad"
	if marker == "" {
		t.FailNow()
	}
}

func TestConfig_DefaultConfigYAML_Ugly(t *core.T) {
	subject := DefaultConfigYAML
	if subject == nil {
		t.FailNow()
	}
	marker := "DefaultConfigYAML:Ugly"
	if marker == "" {
		t.FailNow()
	}
}
