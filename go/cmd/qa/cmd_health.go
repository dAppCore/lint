...
func addHealthCommand(c *core.Core) core.Result {
	return registerQACommand(c, "qa/health", qaText("cmd.qa.health.long"), runHealth)
}

func runHealth() core.Result {
	if !findQAExecutable("gh").OK {
		return core.Fail(core.E(cmdHealthQaHealth2cf50a, qaText("error.gh_not_found"), nil))
	}

	regResult := loadHealthRegistry()
	if !regResult.OK {
		return regResult
	}
...
func loadHealthRegistry() core.Result {
	...
	if healthRegistry != "" {
		...
		if findErr != nil {
			return core.Fail(core.E(cmdHealthQaHealth2cf50a, qaText("error.registry_not_found"), nil))
		}
		reg, err = repos.LoadRegistry(io.Local, registryPath)
	}
	if err != nil {
		return core.Fail(core.E(cmdHealthQaHealth2cf50a, "failed to load registry", err))
	}
	return core.Ok(reg)
}
...
func printHealthSummary(summary HealthSummary, healthResults []RepoHealth) core.Result {
	cli.Print("%s: %d/%d repos healthy (%d%%)\n\n",
		qaText("cmd.qa.health.summary"),
		summary.Passing,
		summary.TotalRepos,
		summary.PassingRate)

	if len(healthResults) == 0 {
		cli.Text(qaText("cmd.qa.health.all_healthy"))
		return core.Ok(nil)
	}
...
func fetchRepoHealth(org, repoName string) RepoHealth {
	...
	if output.Err != nil {
		if core.Contains(output.Stderr, "no workflows") || core.Contains(output.Stderr, "not found") {
			return RepoHealth{
				Name:    repoName,
				Status:  "no_ci",
				Message: qaText("cmd.qa.health.no_ci_configured"),
			}
		}
		return RepoHealth{
			Name:    repoName,
			Status:  "error",
			Message: qaText("cmd.qa.health.fetch_error"),
		}
	}
	...
	if r := core.JSONUnmarshal([]byte(output.Stdout), &runs); !r.OK {
		return RepoHealth{
			Name:    repoName,
			Status:  "error",
			Message: qaText("cmd.qa.health.parse_error"),
		}
	}
	...
	case "completed":
		...
		case "success":
			...
			health.Message = qaText("cmd.qa.health.passing")
		case "failure":
			health.Status = "failing"
			health.Message = qaText("cmd.qa.health.tests_failing")
		case "cancelled":
			health.Status = "pending"
			health.Message = qaText("cmd.qa.health.cancelled")
		case "skipped":
			health.Status = "passing"
			health.Message = qaText("cmd.qa.health.skipped")
		default:
			health.Status = "failing"
	...
	case "in_progress", "queued", "waiting":
		health.Status = "pending"
		health.Message = qaText("cmd.qa.health.running")
	default:
		health.Status = "no_ci"
...
func printHealthJSON(summary HealthSummary, repos []RepoHealth) core.Result {
	data := core.JSONMarshalIndent(HealthOutput{
		Summary: summary,
		Repos:   repos,
	}, "", "  ")
	if !data.OK {
...
func printHealthGroup(status string, repos []RepoHealth, style *cli.AnsiStyle) {
	...
	switch status {
	case "failing":
		label = qaText("cmd.qa.health.count_failing")
	case "error":
		label = qaText("cmd.qa.health.count_error")
	case "pending":
		label = qaText("cmd.qa.health.count_pending")
	case "no_ci":
		label = qaText("cmd.qa.health.count_no_ci")
	case "disabled":
		label = qaText("cmd.qa.health.count_disabled")
	case "passing":
		label = qaText("cmd.qa.health.count_passing")
	}
